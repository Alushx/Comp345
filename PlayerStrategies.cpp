...
int HumanPlayer::pickCard(Hand *hand, int coinsAvailable){
    int index;
...
int GreedyComputer::pickCard(Hand *hand, int coinsAvailable){
    int cheapestCard = 0;
    Action act;
    Card *handcards = hand->getHandCards();
    int cardCost[] = { 0, 1, 1, 2, 2, 3 };
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        handcards[i].getSecondAction() == act.DESTROY_1_ARMY || handcards[i].getSecondAction() == act.BUILD_CITY) && coinsAvailable >= cardCost[i]){
           return i;
        }
    }
...
int ModerateComputer::pickCard(Hand *hand, int coinsAvailable){
    int cheapestCard = 0;
    Action act;
    Ability ability;
    Card *handcards = hand->getHandCards();
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        || handcards[i].getAbility() == ability.PLUS_ONE_MOVE || handcards[i].getAbility() == ability.PLUS_ONE_ARMY ||
            handcards[i].getAbility() == ability.IMMUNE_TO_ATTACK || handcards[i].getAbility() == ability.FYLING) && coinsAvailable >= cardCost[i])
        {
            return i;
        }
    }
